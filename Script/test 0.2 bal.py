...
class GameSim:
    def __init__(self):
        ...
        # THIẾT LẬP BỘ BÀI (Đúng như bản test của bạn)
        self.p_deck = [
    ...
    def auto_choose(self, hand, m_action, state_signal):
        """AI Tự động đưa ra quyết định dựa trên thông tin thu thập được"""
        # Nếu biết chắc chắn là ATK, ưu tiên dùng DEF
        if state_signal == "CUONG_NO":
            def_cards = [c for c in hand if c.type == 'DEF']
            if def_cards: return def_cards[0]
            
        # Nếu không, cố gắng bắt bài Lớp 2 (Style)
    ...
    def run_game(self):
        while True:
            ...
            elif atk_c == 1: state_signal = "E_DE" # Gộp chung cho AI dễ xử lý
...
def run_simulation(runs=1000):
    ...
    print(f"--- KẾT QUẢ MÔ PHỎNG {runs} TRẬN VỚI TÍN HIỆU HÀNH VI ---")
