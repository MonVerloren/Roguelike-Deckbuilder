...
def save_score(name, games_survived):
    if games_survived <= 0: return # Không lưu nếu chưa qua màn nào
    
    board = load_leaderboard()
    board.append({"name": name, "score": games_survived})
    # Sắp xếp giảm dần theo số màn sống sót, lấy top 10
...
def show_tutorial():
    ...
    print("10. Sống sót càng lâu càng tốt! Good Luck!")
...
def play_session(player_name):
    ...
    while True:
        ...
        if status == "WIN":
            ...
            while True:
                nxt = input(Fore.CYAN + "\n(1) Chơi tiếp [Kế thừa chỉ số] \n(2) Thoát về Main Menu?\nVui lòng chọn: ")
                if nxt == '1':
                    game_number += 1
                    # Lưu lại chỉ số để mang sang ván sau
            ...
            # Hiển thị số màn đã sống sót ở ngay dưới dòng chữ GAME OVER
