...
init(autoreset=True)
TYPES = ['ATK', 'DEF', 'SPE']
STYLES = {'Light': 6, 'Combo': 8, 'Heavy': 10}
    ...
    def clash(self, p_card, m_card):
        ...
        if p_card.type == m_card.type:
            if p_card.style == m_card.style:
                # Hòa tuyệt đối -> Nhận nửa tác dụng
                half_val = p_card.value // 2
                print(Fore.YELLOW + f"  -> Hòa tuyệt đối! Cả 2 cùng chịu ảnh hưởng.")
                
                # Tạo thẻ tạm thời với giá trị bị chia đôi để nhét vào hàm apply_effect
                half_card = Card(p_card.type, p_card.style)
                half_card.value = half_val 
                
                self.resolve_effect(half_card, is_player=True, is_x2=False)
                self.resolve_effect(half_card, is_player=False, is_x2=False)
                return
